...
public class AuthenticationServiceImpl implements AuthenticationService {
	...
	public AccessToken lookupToken(String handle)
		...
		return new AccessToken(accessToken.getUser().getId(),
				...
				ImmutableSet.copyOf(Optional
						.fromNullable(accessToken.getScope())
						.transform(String::trim).transform(s -> s.split(" "))
						.or(new String[0])));
	}

	@Override
	public String getAccessToken(long userId, long clientId,
			String clientSecret, String code, String redirectURI)
			throws AuthenticationServiceException {
		try {
			RegisteredClient registeredClient = userDao
			...
			if (StringUtils.isNotBlank(clientSecret)
				...
				if (accessToken.getExpiration() <= 0) {
					throw new UserDaoException(
							"Can't get access token with expired authorize code.");
				}
	...
	@Override
	public String getAuthorizationCode(long userId, long clientId,
			String redirectURI, Set<String> scope)
			throws AuthenticationServiceException {
		...
		try {
			...
			throw new AuthenticationServiceException(
					"Cannot get authorization code", e);
		}
	}

	/**
	 * Check the client is a valid wpg client. <b>NOTE:</b>some client
	 * frameworks don't supply client_id and reply on redirect_uri as the key,so
	 * use redirect_uri as a client identifier.
	 *
	 * @param redirectURI
	 *            client identifier.
	 ...
	 */
	@Override
	public RegisteredClient getClient(String redirectURI)
			throws AuthenticationServiceException {
		try {
			return StringUtils.isNotBlank(redirectURI) ? userDao
					.getClientByRedirectURI(redirectURI) : null;
		} catch (UserDaoException e) {
			throw new AuthenticationServiceException(
					"Cannot get register client by redirect_uri", e);
		}
	}
	...
	@Override
	public int removeUserAvatar(long uid) throws AuthenticationServiceException {
		try {
	...
	@Override
	public User updateUserPwd(User user) throws AuthenticationServiceException {
		try {
