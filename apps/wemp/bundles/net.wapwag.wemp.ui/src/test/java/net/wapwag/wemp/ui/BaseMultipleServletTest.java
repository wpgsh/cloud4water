...
public abstract class BaseMultipleServletTest {

	private Server server;

	private final int port, maxServerThreads, acceptQueueSize;

	public BaseMultipleServletTest(int port, int maxServerThreads, int acceptQueueSize) {
		super();
		this.port = port;
    ...
    private static Server createServer(int port, int maxServerThreads, int acceptQueueSize, Filter filter, Map<String, Servlet> servletMap) {
        ...
        for (String map : servletMap.keySet()) {
            ServletHolder servletHolder = new ServletHolder(servletMap.get(map));
            handler.addServlet(servletHolder, map);
        }
        if (filter != null) {
	...
	abstract Filter createFilter() throws Exception;

	abstract Map<String, Servlet> createMultipleServlet() throws Exception;
	
	abstract WaterEquipmentDao createWaterEquipmentDao() throws Exception;
	
	@Before
	public void setupServer() throws Exception {
		if (port >= 0) {		
			server = createServer(port, maxServerThreads, acceptQueueSize, createFilter(), createMultipleServlet());
			server.start();
		}
	}
